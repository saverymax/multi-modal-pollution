...
# This script generates TS plots for stations of interest for each chemical of interest in Belgium
...
for (chem_index in 1:length(chem_ids)){
    ...
    for (station_id in 1:nrow(selected_stations)){
        ...
        if (nrow(station_subset) > 0 & sum(!is.na(station_subset$Concentration)) > 3){
            ...
            png(filename=paste(user_dir, "/data/figures/belgium_air_ts/", chem, "_", station_name, "_", station_id, ".png", sep=""), width=700)
            #ts.plot(air_ts)
